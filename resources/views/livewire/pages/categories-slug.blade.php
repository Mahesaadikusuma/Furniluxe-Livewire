<div>
    <div class="container max-w-screen-xl px-4 lg:px-0 my-10 ">
        ...
        <div class="flex flex-wrap gap-3 my-5 items-center">
            ...
            <x-categories-link :active="request()->routeIs('categories')" :href="route('categories')">
                All
            </x-categories-link>

            @foreach ($categories as $category)
                {{-- <x-categories-link wire:poll :active="request()->routeIs('category.slug') && request()->route('slug') === $category->slug" :href="route('category.slug', $category->slug)">
                    {{ $category->name }}
                </x-categories-link> --}}

                <x-category.link :href="route('category.slug', $category->slug)" :slug='$category->slug'>
                    {{ $category->name }}
                </x-category.link>
            @endforeach
        </div>
        ...
        <div class="">
            <h1 class="text-3xl font-bold my-5">Products {{ $products->count() }}</h1>
